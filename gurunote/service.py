"""UI 없는 조작 표면 — 설정, 기록, 검색, 내보내기, 노트 편집.

`webui/bridge.py` 의 `Api` 클래스가 이 동작들을 담고 있었다. pywebview 의 `js_api` 로
쓰이는 클래스라서, 창을 띄우지 않는 호출자(CLI, 에이전트, 스크립트)는 접근할 방법이
없었다. 그 중 창이 필요 없는 32 개를 여기로 옮겼다.

`Api` 는 이 클래스를 상속해 window 배선과 실제 대화상자(파일 선택, 저장 위치)만
더한다. 위임 메서드를 나열하지 않으므로 JS 쪽에 노출되는 표면이 그대로 유지되고,
같은 코드가 CLI 와 webview 양쪽에서 돌아간다.

반환 형태는 bridge 시절 그대로다 — 성공은 값이 담긴 dict, 실패는
`{"ok": False, "error": ..., "code": ...}`. JS 뿐 아니라 기계가 읽기에도 맞는 모양이라
바꾸지 않았다.
"""
...
_KNOWN_SETTINGS: tuple[str, ...] = (
    ...
    # 호출 시점마다 읽으므로 저장 즉시 다음 작업부터 반영된다.
...
class GuruNoteService:
    """창 없이 쓸 수 있는 GuruNote 조작 모음.

    상태를 들지 않는다. 필요한 것은 호출 인자와 디스크(설정, 기록, 캐시)뿐이다.
    """
