...
from gurunote.history import (
    ...
    load_index, rebuild_index, update_job_markdown,
)
...
# 파이프라인 워커 (백그라운드 스레드)
# -----------------------------------------------------------------------------
# 구현은 gurunote/pipeline_worker.py 로 이동했다 (backlog B09). React/PyWebView
# 진입점이 이 CustomTkinter 파일을 import 하지 않아도 되게 하려는 분리다.
# 이 이름을 직접 쓰던 기존 코드를 위해 여기서 계속 re-export 한다.
# =============================================================================
from gurunote.pipeline_worker import PipelineWorker  # noqa: E402  (섹션 위치 유지)
...
class GuruNoteApp(ctk.CTk):
    ...
    def _build_sidebar(self):
        ...
        ctk.CTkLabel(
            sb, text="v1.0.0.33", font=ctk.CTkFont(size=10), text_color=C_TEXT_DIM,
